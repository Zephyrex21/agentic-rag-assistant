...
- **User accounts with real per-user data isolation** — sign in via a passwordless 6-digit email code (a first-time email transparently becomes an account on verify — no separate signup step or password to set) or via Google/GitHub OAuth, whichever a deployment has configured. `POST /api/auth/otp/{request,verify}`, `GET/POST /api/auth/oauth/:provider[/callback]`, `POST /api/auth/logout`, `GET /api/auth/me` (also reports which OAuth providers are configured, so the UI only renders working buttons). The session itself is a JWT, sent both as an httpOnly cookie and returned directly in the sign-in response for the client to send as an `Authorization: Bearer` header — the header is what actually matters for a separate-domain deployment, since a cross-site cookie is unreliable there (see `middleware/userAuth.js`'s own comment). OTP codes are sha256-hashed at rest, single-use, expire after 10 minutes, and are capped at 5 attempts before requiring a fresh one; OAuth kickoff/callback routes are deliberately mounted ahead of the `APP_ACCESS_KEY` gate (a real browser navigation can't attach that header — see `app.js`'s comment at the mount point) and validate a short-lived CSRF `state` cookie on the way back. Guest mode (no account at all) works exactly as this app always did, scoped to a shared `user_id IS NULL` pool; a logged-in user's uploads/conversations are tagged with their account and only ever visible to them from then on — enforced at the data layer (documentStore/conversationStore/folderStore's `userId` option, plus a `userId` field on every Pinecone vector and keyword-search chunk row) rather than only in route handlers, so the isolation holds even if a route forgot to check. See `migration_007_users_and_ownership.sql`, `migration_008_email_otp_auth.sql`, `middleware/userAuth.js`, and `services/oauthProviders.js`.
