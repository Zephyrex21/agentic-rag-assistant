...
export function Hero({ onEnter }: { onEnter: () => void }) {
  ...
  return (
    <div className="relative flex flex-col">
      {/* Confined to exactly the first viewport (h-svh, not min-h-svh) so
          the 3D signal-field background below doesn't stretch across the
          new FeaturesSection that now follows it - the scene is meant to
          fill one screen, not the whole scrollable page. */}
      <div className="relative flex h-svh flex-col overflow-hidden">
        ...
        <main className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 text-center">
          ...
          >
            ...
            >
              Agentic Retrieval · Hybrid Search · Verified Citations
            </motion.span>
            ...
            >
              ...
              <br className="hidden sm:block" /> Verify every answer.
            </motion.h1>
            ...
            >
              A tool-calling agent plans its own retrieval, fuses hybrid vector + keyword search, reranks
              with an LLM, and checks each claim against source before you ever see it — so every answer
              traces back to an exact citation, not a guess.
            </motion.p>
          ...
          {/* A quiet nudge that there's more below the fold, now that the
              hero is followed by FeaturesSection - without this, a fixed
              h-svh hero with no visible scroll affordance can read as the
              whole page. */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.6, delay: 0.9 }}
            className="pointer-events-none absolute bottom-8 left-1/2 hidden -translate-x-1/2 flex-col items-center gap-1.5 text-ink-muted sm:flex"
          >
            <span className="font-mono text-[10px] uppercase tracking-[0.2em]">How it works</span>
